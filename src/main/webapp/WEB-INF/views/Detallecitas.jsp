...
<div class="total" style="    position: absolute;
    ...
    left: 30%;
    font-size: 28px;">          
         Su total es: S/ ${bsubtotal}
         </div>     
<div  class="container" > 

    <table id="table" style="padding: 0%;position: absolute;margin: -109px;margin-left: -500px;">
      <thead>
        <tr>
      ...
      <tbody>
        <c:forEach var="detalle" items="${bDetalle}">
		<tr>
					<td>${detalle.ordenpedido.cliente.cod_Cliente}</td>
					<td>${detalle.ordenpedido.cliente.dni}</td>
					<td>${detalle.ordenpedido.cliente.nombres}</td>
					<td>${detalle.ordenpedido.fecha}</td>
					<td>${detalle.ordenpedido.cod_pedido}</td>
					<td>${detalle.cantidad}</td>
					<td>S/ ${detalle.subtotal}</td>
					<td>${detalle.articulo.codArticulo}</td>
					<td>
								<c:set var="typeImage" value="${fn:substringAfter(detalle.articulo.imagen,'.')}" />
				
								<img src="data:image/${typeImage};base64,${detalle.articulo.getBase64Imagen()}" width="120" height="120"/>
							</td>
